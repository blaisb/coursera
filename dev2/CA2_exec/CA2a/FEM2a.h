    ...
    for (uint i = 0; i < totalNodes ; ++i)
    {
        ...
        if (alq(nodeLocation[i][1],0.08))
        {
            ...
            boundary_values[i] = 310*(1.+8.*nodeLocation[i][0]*nodeLocation[i][0]);
                ...
                // Left Dirichlet
        if (alq(nodeLocation[i][0],0.0))
        {

//            boundary_values[i] = 300;//*(1.+1./3.*nodeLocation[i][0]*nodeLocation[i][0]);
        }

        // RIGHT
        if (alq(nodeLocation[i][0],0.03))
        {

//            boundary_values[i] = 310;//*(1.+1./3.*nodeLocation[i][0]*nodeLocation[i][0]);
        }
    }
                        ...
                        for(unsigned int j=0;j<dim;j++){
                            for(unsigned int I=0;I<dim;I++){
                                for(unsigned int J=0;J<dim;J++){
                                    ...
                                    Klocal[A][B] += basis_gradient(A,xi1, xi2)[i] * invJacob[i][I] * kappa[I][J]
                                                    * basis_gradient(B,xi1,xi2)[j] * invJacob[j][J]
                                                    *weight *detJ;
                                }
                            }
                        }
